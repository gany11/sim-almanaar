...
<div class="container mx-auto pb-10">
    <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <a href="<?= base_url('admin/akun/registrasi') ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-lg shadow-blue-200 transition-all flex items-center justify-center gap-2">
    ...
    <?php if (session()->getFlashdata('sukses')) : ?>
        <div class="mb-6 p-4 bg-emerald-50 border-l-4 border-emerald-500 text-emerald-700 rounded-xl flex items-center gap-3 shadow-sm">
            ...
            <span class="text-sm font-medium"><?= session()->getFlashdata('sukses') ?></span>
        </div>
    <?php endif; ?>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    /**
     * Fungsi utama untuk memuat data tabel via AJAX
     */
    const loadData = () => {
    ...
    /**
     * Event Delegation untuk tombol Nonaktifkan/Pulihkan
     * Menggunakan delegasi karena tombol dibuat secara dinamis
     */
    $(document).on('click', '.btn-toggle-status', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "<?= base_url('admin/akun/status') ?>",
                    type: "POST",
                    data: {
                        id_akun: id,
                        status: targetStatus,
