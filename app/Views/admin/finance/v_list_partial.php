                        ...
                        <div class="flex flex-col">
                            ...
                            <div class="mt-1.5 space-y-0.5">
                                <span class="text-[9px] text-blue-400 block italic">
                                    ...
                                    Dibuat: <?= format_indo($row['created_at'], 'full_datetime') ?> oleh <span class="font-semibold text-gray-500"><?= $row['creator_name'] ?? 'Sistem' ?></span>
                                </span>
                                <?php if ($row['updated_at'] && $row['updated_by']): ?>
                                <span class="text-[9px] text-blue-400 block italic">
                                    <i class="inline-block w-1.5 h-1.5 bg-blue-400 rounded-full mr-1"></i>
                                    Terakhir diubah: <?= format_indo($row['updated_at'], 'full_datetime') ?> oleh <span class="font-semibold text-gray-500"><?= $row['editor_name'] ?? 'User' ?></span>
                                </span>
                                <?php endif; ?>
                            </div>
                        </div>
            ...
            <?php foreach ($history as $h): ?>
                ...
                <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    <td class="px-6 py-4">
                        ...
                        <span class="text-[10px] text-gray-400 italic">Periode: <?= date('d/m/y', strtotime($h['started_at'])) ?> - <?= date('d/m/y', strtotime($h['ended_at'])) ?></span>
                    </td>
                    <td class="px-6 py-4 text-center">
